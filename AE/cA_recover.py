# -*- coding: utf-8 -*-
"""
Created on Tue Mar 01 22:31:17 2016
...
#            initial_W = numpy.asarray(0.02 * numpy_rng.randn(n_visible, n_hidden),dtype=theano.config.floatX)
    ...
    def get_cost_updates(self, contraction_level, learning_rate):
        ...
        #z = T.clip(z, 0.0001, 0.9999)
        J = self.get_jacobian(y, self.W)
        #J = theano.gradient.jacobian(y.sum(), self.W)
...
##            f = direc+'/'+dir_[0]
            f = 'D:/Research/nUnqSig_7-durEachSig_1000ms-nOccurEachSig_3-fBW_2000mHz-tBW_31ms-amp_94dB-prcOvrlp_94-noNoise_nPSD'
    ...
    #        writeacc.write(pivot+'\n')
            for hid in hids:
                hid = hid
                savename = 'cA_'+ fname + '_'+toneorchord+'_' + str(hid)
#                print 'hidden nodes,', hid
                #train_num = 3000
                output_folder = 'D:/Dropbox/DL/DeepMSS/Report/nPSD_tone/'
                ...
                for epoch in xrange(training_epochs):
                    ...
                    if printmd: print 'Training epoch %d, reconstruction cost ' % epoch, numpy.mean(
...
#                                      ' ran for %.2fm' % ((training_time) / 60.))
                ...
                image = PIL.Image.fromarray(tile_raster_images(
                    ...
                    tile_spacing=(1, 1)))
#                image = PIL.Image.fromarray(tile_raster_images(
#                    X=numpy.flipud(ca.W.get_value(borrow=True)).T.reshape(-1,hid),
#                    img_shape=(31, 39), tile_shape=(1, hid),
#                    tile_spacing=(1, 1)))                
                cbarmin = np.min(ca.W.get_value(borrow=True))
                cbarmax = np.max(ca.W.get_value(borrow=True))
                #image.save('cae_filters.png')
                #savename = 'CAE_'+ fname + 'chord_'+str(chord)+str(hid)
...
##savecsv = tile_raster_images( X=numpy.flipud(ca.W.get_value(borrow=True)).T, img_shape=(height/31, length*31), tile_shape=(1, hid), tile_spacing=(1, 1))/256.;np.savetxt('D:\Dropbox\DL\DeepMSS\Report\cA-21 hidden units-tone dataset.csv',savecsv)                
#                plt.figure(figsize=(23.0, 16.0));plt.imshow(image);plt.title(savename);plt.colorbar()
                ...
                savecsv = tile_raster_images( X=numpy.flipud(ca.W.get_value(borrow=True)).T, img_shape=(height/32, length*32), tile_shape=(1, hid), tile_spacing=(1, 1))/256.;
...
#                print 'Saving the weights'
                if not os.path.isdir('cA_weights_chord/'):
                    os.makedirs('cA_weights_chord/')
                ...
                feature_set = spe.expit(numpy.dot(train_set_x_full, ca.W.get_value()) +ca.b.get_value())
...
#                print 'test score:', testacc
                print hid, '\t',numpy.mean(c_array[:,0]), '\t', trainacc
                rec_z = spe.expit(numpy.dot(feature_set, ca.W.get_value().T) +ca.b_prime.get_value())
        ...
        #        writeacc.write(str(hid)+'\t'+str(trainacc)+'\t'+str(testacc)+'\n')
                ...
                #Generate .arff file
                if not os.path.isdir('cA_arff_chord/'):
                    os.makedirs('cA_arff_chord/')
...
xx[:12,:]=0
xx[13:,:]=0
yy = spe.expit(numpy.dot(xx, ca.W.get_value()) +ca.b.get_value())
zz= spe.expit(numpy.dot(yy, ca.W.get_value().T) +ca.b_prime.get_value())
savematxx = tile_raster_images( X=xx, img_shape=(height/32, length*32), tile_shape=(1, 21), tile_spacing=(1, 1)); savematzz= tile_raster_images( X=zz, img_shape=(height/32, length*32), tile_shape=(1, 21), tile_spacing=(1, 1))
imagexx = PIL.Image.fromarray(savematxx)
imagezz = PIL.Image.fromarray(savematzz)
#np.savetxt(output_folder + savename +'_raw_1tone.csv',savemat1/256., delimiter=',')
#np.savetxt(output_folder + savename +'_rec_1tone.csv',savemat1/256., delimiter=',')
plt.figure(figsize=(23,16));plt.subplot(211);plt.title('raw signal');plt.imshow(imagexx, extent=[0,21,pfmin*3,pfmax*3]);plt.subplot(212);plt.title('reconstructed signal');plt.imshow(imagezz, extent=[0,21,pfmin*3,pfmax*3])
#%%
