...
for chord in chords:
    ...
    #        writeacc.write(pivot+'\n')
            for hid in hids:
                ...
                batch_size = int(np.ceil(train_set_x_full.shape[1]/10.))
                
                length = 1
                height = input_size
                
                test_set_x = train_set_x_full.T[:]
                train_set_x = train_set_x_full.T[:]
                ...
                fig, ax = plt.subplots(figsize=(23,16));im = ax.imshow(image, extent=[0,hid,mintick,maxtick]);plt.title(savename)
